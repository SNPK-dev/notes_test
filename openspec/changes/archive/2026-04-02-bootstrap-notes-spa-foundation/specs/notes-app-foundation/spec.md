...
Система SHALL предоставлять на editor page архитектурно закреплённые точки входа для create, edit, cancel, undo и redo сценариев. Для foundation-этапа cancel MUST быть поддержан как пользовательский сценарий, а undo/redo MUST быть зарезервированы как editor actions и keyboard shortcuts, даже если полный history-механизм будет дорабатываться в следующих changes.
...
- **THEN** система вызывает editor-level действие undo, а не пытается реализовать откат внутри presentational component

#### Scenario: Redo action is available through editor contract
- **WHEN** пользователь инициирует redo на странице редактора
- **THEN** система вызывает editor-level действие redo, а не пытается реализовать повтор внутри presentational component
